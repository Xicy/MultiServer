...
namespace Shared.Security
{
    ...
    public class Blowfish : ICrypter
    {
        ...
        public void EncodeBuffer(ref byte[] buffer)
        {
            Array.Resize(ref buffer, buffer.Length + (8 - (buffer.Length % 8)));
            Encipher(buffer, buffer.Length);
        }

        public void DecodeBuffer(ref byte[] buffer)
        {
            //Array.Resize(ref buffer, buffer.Length + (8 - (buffer.Length % 8)));
            Decipher(buffer, buffer.Length);
        }
